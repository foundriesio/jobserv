...
class ProjectDefinition(object):
                    ...
                    for j, combo in enumerate(itertools.product(*values)):
                        ...
                        for t in r.get("triggers", []):
                            ...
                            if rname:
                                ...
                                # rather than:
                                #   {{name}}-{loop}
                                t["run-names"] = rname.format(name="{name}", loop=name)
    ...
    def get_run_definition(self, dbrun, run, trigger, params, secrets):
        url = url_for(
            "api_run.run_update",
            proj=dbrun.build.project.name,
            build_id=dbrun.build.build_id,
            run=dbrun.name,
            _external=True,
        )
        public = url
        if RUN_URL_FMT:
            public = RUN_URL_FMT.format(
        ...
        rundef = {
            ...
            "run_url": url,
            "frontend_url": public,
            "runner_url": url_for("api_worker.runner_download", _external=True),
        ...
        if "script" in run:
            ...
            rundef["script-repo"] = self.script_repos[run["script-repo"]["name"]].copy()
            ...
            for token in (token or "").split(":"):
                ...
                if token and not val:
                    err = (
                        "The script-repo requires a token(%s) not defined "
        ...
        if rundef["container-auth"]:
            if rundef["container-auth"] not in secrets:
                err = (
                    '"container-auth" requires a secret(%s) not '
