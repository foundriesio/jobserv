...
def build_url(build):
    if BUILD_URL_FMT:
        return BUILD_URL_FMT.format(project=build.project.name, build=build.build_id)

    return url_for(
        "api_build.build_get", proj=build.project.name, build_id=build.build_id
    )


def run_url(run):
    if RUN_URL_FMT:
        return RUN_URL_FMT.format(
            project=run.build.project.name, build=run.build.build_id, run=run.name
        )
...
def smtp_session():
    ...
    if rv != 220:
        log.error(
            "Unable to connect to SMTP server %s: %d %s", SMTP_SERVER, rv, msg.decode()
        )
        raise ApiError(500, "Unable to connect to SMTP server")
    rv, msg = s.login(SMTP_USER, SMTP_PASSWORD)
    if rv != 235:
        log.error(
            "Unable to authenticate with SMTP server %s: %d %s",
            SMTP_SERVER,
            rv,
            msg.decode(),
        )
